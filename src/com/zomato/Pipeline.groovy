...
def helmDeploy(Map args) {
    ...
    if (args.dry_run) {
        ...
        sh "helm install --dry-run --debug ${args.chart_dir} --set imageTag=${args.version_tag},replicas=${args.replicas},cpu=${args.cpu},memory=${args.memory} --namespace=${namespace}"
    } else {
        println "Running deployment"

        // reimplement --wait once it works reliable
        sh "helm upgrade --install ${args.name} ${args.chart_dir} --set imageTag=${args.version_tag},replicas=${args.replicas},cpu=${args.cpu},memory=${args.memory} --namespace=${namespace}"
...
def containerBuildPub(Map args) {
    ...
    docker.withRegistry("https://${args.host}", "${args.auth_id}") {
        ...
        sh "docker build --build-arg VCS_REF=${env.GIT_SHA} --build-arg BUILD_DATE=`date -u +'%Y-%m-%dT%H:%M:%SZ'` -f ${args.dockerfile}  -t ${args.acct}/${args.repo} ."
